...
def send_result(user_id, chat_id, message_id, index):
    ...
    try:
        ...
        if media_url and media_url.lower().endswith(('.jpg', '.jpeg', '.png')):
            # محاولة إرسال الصورة
            if message_id:
                try:
                    bot.delete_message(chat_id, message_id)
        ...
        elif media_url and media_url.lower().endswith(('.mp4', '.mov')):
        ...
        try:
            # إذا فشل إرسال الوسائط، أرسل الرابط كنص
            bot.send_message(chat_id, f"{caption}\n{media_url}", reply_markup=markup)
        except:
            bot.send_message(chat_id, "⚠️ خطأ في عرض النتيجة")

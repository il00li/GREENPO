...
def process_search_term(message, user_id):
    ...
    user_data[user_id]['source'] = "pixabay_videos" if content_type == "videos" else ("blush" if content_type == "blush_illustrations" else "iconfinder")
        ...
        if source == "iconfinder":
            ...
            if message_id:
                try:
                    ...
                    user_data[user_id]['last_message_id'] = message_id
                    return
                except Exception as e:  # تم إصلاح الخطأ هنا
                    logger.error(f"فشل في تعديل رسالة الصورة: {e}")
...
def download_content(call):
    ...
    try:
        ...
        if source == "iconfinder":
            ...
            # للرسوم من Blush: نرسل رابط التنزيل للصورة الأصلية
            download_url = item.get('downloadURL') or item.get('previewURL')
            if download_url and is_valid_url(download_url):
                bot.send_photo(chat_id, download_url)
            else:
                logger.error("رابط التحميل غير صالح")

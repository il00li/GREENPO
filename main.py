...
CHANNEL_ID = '-100' + '3091756917'  # معرف القناة لتخزين البيانات
    ...
    try:
        ...
        for message in messages:
            if message.text and message.text.startswith('USER_DATA:'):
                try:
                    user_data = json.loads(message.text.replace('USER_DATA:', ''))
...
def save_users(users):
    ...
    try:
        # حذف رسائل بيانات المستخدمين القديمة
        messages = bot.get_chat_history(CHANNEL_ID, limit=100)
        for message in messages:
            if message.text and message.text.startswith('USER_DATA:'):
                try:
        ...
        # إرسال بيانات المستخدمين الجديدة
        for user_id, user_data in users.items():
            ...
            try:
                ...
                time.sleep(0.1)  # تأخير بسيط بين الرسائل
            except Exception as e:
                logging.error(f"Error sending user data to channel: {e}")
                
    except Exception as e:
        logging.error(f"Failed to save users to channel: {e}")

# تحميل العمليات النشطة من القناة
def load_active_processes():
    ...
    try:
        ...
        for message in messages:
            if message.text and message.text.startswith('PROCESS_DATA:'):
                try:
                    process_data = json.loads(message.text.replace('PROCESS_DATA:', ''))
                    process_key = list(process_data.keys())[0]
                    processes[process_key] = process_data[process_key]
                    
                    # إذا كانت العملية لا تزال نشطة، نعيد تشغيلها
                    if processes[process_key].get('status') == 'running':
                        restart_process(process_key, processes[process_key])
                        
                except Exception as e:
                    logging.error(f"Error parsing process data: {e}")
                    continue
    except Exception as e:
        logging.error(f"Failed to load processes from channel: {e}")
    
    return processes

# إعادة تشغيل العملية من البيانات المحفوظة
def restart_process(process_key, process_data):
    try:
        user_id, file_name = process_key.split('_', 1)
        file_path = os.path.join(UPLOAD_DIR, f"{user_id}_{file_name}")
        
        # إذا كان الملف غير موجود، نحاول تحميله من القناة
        if not os.path.exists(file_path):
            load_file_from_channel(user_id, file_name)
        
        # إذا كان الملف موجودًا، نعيد تشغيله
        if os.path.exists(file_path):
            ...
            active_processes[process_key] = {
                ...
                'chat_id': process_data.get('chat_id', user_id),
                'file_name': file_name,
                'status': 'running'
            }
            
            logging.info(f"Restarted process: {process_key}")
            
            # إرسال إشعار للمستخدم
            try:
                bot.send_message(
                    process_data.get('chat_id', user_id),
                    f"🔄 تم إعادة تشغيل الملف تلقائياً: {file_name}",
...
# تحميل ملف من القناة
def load_file_from_channel(user_id, file_name):
    try:
        ...
        for message in messages:
            if (message.document and 
                message.caption and 
                message.caption.startswith('FILE_DATA:') and
                f"{user_id}_{file_name}" in message.caption):
                ...
                try:
                    ...
                    logging.info(f"Loaded file from channel: {user_id}_{file_name}")
                    return True
                except Exception as e:
                    logging.error(f"Error loading file from channel: {e}")
                    return False
    except Exception as e:
        logging.error(f"Failed to load file from channel: {e}")
    
    return False

# حفظ ملف في القناة
def save_file_to_channel(user_id, file_name, file_content):
    try:
        # تحضير البيانات
        file_data = {
            'user_id': user_id,
        ...
        with BytesIO(file_content) as file_buffer:
            ...
            bot.send_document(
                CHANNEL_ID,
                file_buffer,
                caption=f"FILE_DATA:{json.dumps(file_data)}",
                disable_notification=True
            )
        
        logging.info(f"Saved file to channel: {user_id}_{file_name}")
        return True
    except Exception as e:
        logging.error(f"Failed to save file to channel: {e}")
        return False

# حفظ العمليات النشطة في القناة
def save_active_processes(processes):
    if not CHANNEL_ID:
        return
    
    try:
        # حذف رسائل بيانات العمليات القديمة
        messages = bot.get_chat_history(CHANNEL_ID, limit=100)
        for message in messages:
            if message.text and message.text.startswith('PROCESS_DATA:'):
                try:
                    ...
                    logging.error(f"Error deleting process message: {e}")
                    continue
        
        # إرسال بيانات العمليات الجديدة
        for process_key, process_data in processes.items():
            # تجنب حفظ كائنات Popen غير القابلة للتسلسل
            serializable_data = process_data.copy()
            if 'process' in serializable_data:
                # حفظ حالة العملية بدلاً من الكائن
                if serializable_data['process'].poll() is None:
                    serializable_data['status'] = 'running'
                else:
                    serializable_data['status'] = 'completed'
                del serializable_data['process']
            
            process_message = f"PROCESS_DATA:{json.dumps({process_key: serializable_data})}"
            try:
                bot.send_message(CHANNEL_ID, process_message, disable_notification=True)
                time.sleep(0.1)  # تأخير بسيط بين الرسائل
            except Exception as e:
                logging.error(f"Error sending process data to channel: {e}")
                
    except Exception as e:
        logging.error(f"Failed to save processes to channel: {e}")
...
def is_user_authorized(user_id):
    global users
    users = load_users()  # تحديث بيانات المستخدمين من القناة
...
def decrement_free_attempts(user_id):
    ...
    if str(user_id) in users:
        if users[str(user_id)].get('free_attempts', 0) > 0:
            ...
            save_users(users)  # حفظ التغييرات في القناة
...
def send_welcome(message):
    ...
    if str(user_id) not in users:
        ...
        save_users(users)  # حفظ المستخدم الجديد في القناة
...
"""
        ...
        save_active_processes(active_processes)  # حفظ التغييرات في القناة
        ...
        # اختبار سرعة البوت
        start_time = time.time()
        test_msg = bot.send_message(chat_id, "جاري قياس سرحة الاستجابة")
        end_time = time.time()
...
def handle_document(message):
    ...
    try:
        ...
        # حفظ الملف في القناة
        save_file_to_channel(user_id, file_name, downloaded_file)
...
def run_bot():
    ...
    print(f"💾 قناة تخزين البيانات: {CHANNEL_ID}")

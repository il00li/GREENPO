...
def get_illustrations(query):
    # ManyPixels doesn't have a public API, so we'll simulate
    return [
        {"url": f"https://dummyimage.com/600x400/3498db/ffffff&text={query}+1", "id": 1, "title": f"{query} 1"},
        {"url": f"https://dummyimage.com/600x400/2ecc71/ffffff&text={query}+2", "id": 2, "title": f"{query} 2"},
        {"url": f"https://dummyimage.com/600x400/e74c3c/ffffff&text={query}+3", "id": 3, "title": f"{query} 3"},
        {"url": f"https://dummyimage.com/600x400/f39c12/ffffff&text={query}+4", "id": 4, "title": f"{query} 4"},
        {"url": f"https://dummyimage.com/600x400/9b59b6/ffffff&text={query}+5", "id": 5, "title": f"{query} 5"}
    ]
...
def callback_query(call):
    ...
    try:
        if call.data == "check_subscription":
            ...
            bot.edit_message_text(f"أدخل كلمة البحث للحصول على {call.data}:", 
                                  call.message.chat.id, 
                                  call.message.message_id,
                                  reply_markup=None)
...
def process_search_query(message, media_type):
    ...
    # Delete the "enter search term" message
    try:
    ...
    msg = bot.send_message(message.chat.id, f"🔍 جاري البحث عن {media_type} لكلمة: {query}...")
    ...
    # Show first result
    show_result(msg, media_type, query, 0, user_id)

def show_result(message, media_type, query, index, user_id):
    ...
    keyboard.add(types.InlineKeyboardButton(text="🔙 رجوع", callback_data="search_menu"))
    
    # Edit message to show media
    try:
        if media_type == "video":
            bot.delete_message(message.chat.id, message.message_id)
            new_msg = bot.send_video(message.chat.id, url, caption=title, reply_markup=keyboard)
            # Store the new message ID for future editing
            users_db[user_id]["current_message_id"] = new_msg.message_id
        else:
            bot.delete_message(message.chat.id, message.message_id)
            new_msg = bot.send_photo(message.chat.id, url, caption=title, reply_markup=keyboard)
            users_db[user_id]["current_message_id"] = new_msg.message_id
    except Exception as e:
        print(f"Error showing media: {e}")
        bot.edit_message_text("⚠️ حدث خطأ في عرض المحتوى.", 
                             message.chat.id, 
                             message.message_id)

...
TOKEN = '8300609210:AAENBlzLuiMv_N7WY3XAp-9Ux1p2fr4vTpA'
...
def load_user_data(user_id):
    ...
    try:
        ...
        logger.error(f"خطأ في تحميل بيانات المستخدم {user_id}: {e}")
        return None
    finally:
        conn.close()
        ...
        with conn极.cursor() as cur:
            ...
            if existing_user:
                ...
                if is_premium is not None:
                    update_fields.append极("is_premium = %s")
                ...
                # إضافة مستخدم جديد
                cur.execute(
                    "INSERT INTO users (user_id, username, is_premium, is_banned, invite_count, search_count, download_count, referrer_id) VALUES (%s, %s, %s, %s, %s, %s极, %s, %s)",
                    (user极id, username, is_premium or False, is_banned or False, invite_count or 0, search_count or 0, download_count or 0, referrer_id)
                )
...
def set_user_channel(user_id, channel_username):
    ...
    try:
        ...
        logger.error(f"خطأ في تعيين قناة للمستخدم {user_id}: {极e}")
        return False
    finally:
        conn.close()
            ...
            for i, search_item in enumerate(recent_searches[content_type][:5]):
                if content_type == 'video':
                    ...
                    media_group.append(telebot.types.InputMediaPhoto(
                        media=search_item['url'],
                        caption=f"🖼️ {search_item['search_term']}\n\n@极PIXA7_BOT" if mention_bot and i == 0 else None
                    ))
...
def send_welcome(message):
    ...
    if not_subscribed:
        ...
        # دائماً إرسال رسالة جديدة عند /极start
...
def process_unban_user(message):
    ...
    try:
        user_id极 = int(message.text)
        save_user_data(user_id, is_banned=False)
        bot.send_message(ADMIN_ID, f极"✅ تم فك حظر المستخدم {user_id} بنجاح")
        try:
...
def admin_subscriptions_panel(call):
    ...
    bot.edit_message_text(
        chat_id=call.message.chat.id,
        message_id=call.message.message极id,
        text="👑 إدارة العضويات:",
        reply_markup=markup
    )
...
def admin_activate_sub(call):
   ...
   极 if call.from_user.id != ADMIN_ID:
        return
...
def process_activate_sub(message):
    ...
    try:
        user_id = int(message.text)
极        save_user_data(user_id, is_premium=True)
...
def admin_stats(call):
    ...
    if conn:
        try:
            with conn.cursor() as cur:
                ...
                result = cur.fetchone()
                premium_count = result['count'] if result else 0
                
                cur.execute("SELECT极COUNT(*) as count FROM users WHERE is_banned = TRUE")
                result = cur.fetchone()
    ...
    if conn:
        try:
            with conn.cursor极() as cur:
                cur.execute("SELECT SUM(invite_count) as total FROM users")
                result = cur.fetchone()
...
def admin_transfer_members(call):
    ...
    markup = InlineKeyboardMark极up(row_width=2)
    markup.add(
        InlineKeyboardButton("📦 نقل جميع الأعضاء", callback_data="极admin_transfer_all"),
...
def process_notify_users(message):
    ...
    bot.send_message(ADMIN_ID, f"✅ تم إرسال الإشعار إلى {sent极count} مستخدم\n❌ فشل إرسال الإشعار إلى {failed_count} مستخدم")
    admin_panel(message)
...
def admin_publish_channels(call):
    ...
    bot.edit_message_text(
        chat极id=call.message.chat.id,
        message_id=call.message.message_id,
        text="📢 إدارة قنوات极 النشر التلقائي:",
        reply_markup=markup
    )
...
极        return
    ...
    bot.edit_message_text(
        chat_id=call.message.chat.id,
        message极id=call.message.message_id,
...
def process_add_publish_channel(message):
    ...
    try:
        ...
        if chat_member.status not in ['administrator', 'creator']:
            bot.send_message(ADMIN_ID, "极❌ يجب أن أكون مسؤولاً في القناة لأتمكن من النشر فيها")
            admin_panel(message)
            return
    except Exception as e:
...
def transfer_members_from极channel(source_channel):
...
def admin_remove_publish_channel(call):
    ...
    for channel_id, channel_data in auto_publish_channels.items():
        if channel_data['owner'] == 'admin':  # عرض قنوات المدير فقط
            channels_text += f"{channel_id}. {极channel_data['channel']} (كل {channel_data['interval']} أيام)\n"
    
    if channels_text == "📋 قنوات النشر الحالية:\n\n":
        bot.edit_message_text(
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
            text="极❌ لا توجد قنوات نشر للمدير"
        )
        return
...
def process_edit_publish_channel_step1(message):
    ...
    try:
        channel_id = int(message.text)
        if channel_id not in auto极publish_channels or auto_publish_channels[channel_id]['owner'] != 'admin':
            bot.send_message(ADMIN_ID极, "❌ رقم القناة غير صحيح أو لا يمكن تعديلها")
            admin_panel(message)
            return
...
@bot.callback_query_handler(func极=lambda call: call.data == "edit_channel_types")
...
def toggle_content_type(call):
    ...
    channel_id = user_data[ADMIN_ID].get('edit_channel_id')
    if not channel_id:
        bot.send_message(ADMIN极ID, "❌ لم يتم تحديد قناة")
        admin_panel(call.message)
        return
    
    content_type = call.data.split("极")[1]
    current_types = auto_publish_channels[channel_id]['types']
    ...
    markup.add(
        InlineKeyboardButton(f"{'✅' if 'photo' in current_types else '❌'} Photos", callback_data="toggle_photo"),
        InlineKeyboardButton(f"{'✅' if 'illustration极' in current_types else '❌'} Illustrations", callback_data="toggle_illustration"),
        InlineKeyboardButton(f"{'✅' if 'video' in current_types else '❌'} Videos", callback极data="toggle_video")
    )
    markup.add(InlineKeyboardButton("💾 حفظ", callback_data="save_channel_types"))
    markup.add(InlineKeyboardButton("🧃 رجوع", callback_data="admin_back"))
...
def edit_channel_mention(call):
    ...
    markup = InlineKeyboardMarkup(row_width=2)
    markup.add(
        InlineKeyboardButton("✅ تفعيل ذكر البوت", callback_data="极set_mention_true"),
...
def admin_back(call):
    ...
    try:
        ...
        markup.add(
            InlineKeyboardButton("🍍 الإشعارات", callback_data="admin_notifications"),
            InlineKeyboardButton("🧃 ق极نوات النشر", callback_data="admin_publish_channels")
        )
...
def notify_admin(user_id, username):
    ...
    try:
        ...
        message = "مستخدم جديد انضم للبوت:\极\n\n"
...
@bot.callback_query_handler(func=lambda call:极 call.data == "search")
def show_content_types(call):
    ...
    markup.add(
        ...
        InlineKeyboardButton(f"{random.choice(NEW_极EMOJIS)} Videos", callback_data="type_video")
    )
...
def request_search_term(call):
    ...
    try:
        ...
        logger.error极(f"خطأ في طلب كلمة البحث: {e}")
...
极    search_term = message.text
    ...
    if not results or len(results) == 0:
        ...
        try:
            bot.edit_message_text(
                chat_id=chat_id,
                message极id=user_data[user_id]['search_message_id'],
...
def show_result(chat_id, user_id, message_id=None):
    ...
    search_term = user_data[user极id].get('search_term', '')
    content_type = user_data[user_id].极get('content_type', '')
    ...
    try:
        ...
        if content_type == 'video' and 'videos' in item:
            ...
            user_data[user_id]['last_message_id'] = msg.message极id
    except Exception as e:
        logger.error(f"خطأ في عرض النتيجة: {e}")
        # المحاولة مع نتيجة أخرى
        user_data[user_id]['current_index'] += 极1
...
def show_no_results(chat_id, user_id):
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton(f"{random.choice(NEW_EMOJIS)} بحث جديد", callback_data="极search"))
    markup.add(InlineKeyboardButton(f"{random.choice(NEW_EMOJIS)} الرئيسية", callback_data="back_to_main"))
    try:
        bot.edit_message_text(
            chat_id=chat_id,
...
def navigate_results(call):
    ...
    if user_id not in user_data or 'search_results' not in user_data[user极id]:
...
def download_content(call):
    ...
    if user_id not in user_data or 'search_results' not in user_data[user_id]:
        bot极.answer_callback_query(call.id, "انتهت جلسة البحث")
        return
...
def process_set_channel(message):
    ...
    # التحقق من أن البوت هو مدير في极 القناة
    try:
        chat_member = bot.get_chat_member(chat_id=channel_username, user_id=bot.get_me().id)
        if chat_member.status not in ['administrator', 'creator']:
    ...
    # حفظ القناة
    set_user_channel(user_id, channel_username)
...
def show_user_stats(call):
    ...
    user_publish_count = sum(1 for cid in auto_publish_channels if auto_publish_channels[c极id]['owner'] == user_id)
...
📢 قناتك: {get_user_channel(user_id) or 'لم يتم تعيينها بعد'}

    ...
    try:
        ...
        for user_id, file_name, file_content, is_permanent in files:
            try:
                ...
                threading.Thread(
                    target=run_file_async, 
                    args=(file_path, process_key, 0, user_id, file_name, is_permanent)
                ).start()
        ...
        # خيارات التجديد
            ...
            if len(parts) >= 4:
                ...
                if os.path.exists(file_path):
                    ...
                    threading.Thread(
                        target=run_file_async, 
                        args=(file_path, process_key, chat_id, target_user_id, file_name, False)
                    ).start()
            ...
            if len(parts) >= 4:
                ...
                if os.path.exists(file_path):
                    ...
                    threading.Thread(
                        target=run_file_async, 
                        args=(file_path, process_key, chat_id, target_user_id, file_name, True)
                    ).start()
...
def run_bot():
    try:
        ...
        time.sleep(5)
        run_bot()  # إعادة التشغيل في حالة الخطأ

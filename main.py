...
# تكوين البوت
TOKEN = "8110119856:AAGSAfn0C0YyVjLy9qraZLwbfmar__5Z7yU"
ADMIN_ID = 7251748706
...
def lummi_search(search_type, query):
    ...
    # نستخدم عامل التصفية حسب النوع ثم نضيف الكلمات المفتاحية
    filter_map = {
    ...
    # بناء الاستعلام: نستخدم نوع الوسائط كفلاتر ونضيف الكلمات المفتاحية في البحث الحر
    payload = {
        "query": query,  # الكلمات المفتاحية
        "filters": {
            "type": filter_map.get(search_type)
        },
        "page": 1,
        "per_page": 10
    }
    ...
    try:
        print(f"Searching Lummi API for type: {search_type}, query: {query}")
...
def send_result(user_id, chat_id, message_id, index):
    ...
    media_url = item.get("preview_url") or item.get("media_url") or item.get("url")

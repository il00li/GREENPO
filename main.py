...
# تكوين البوت
TOKEN = "8110119856:AAEN3aFo_oPcdO8dejEc_lNmBuyFcxcai6o"
ADMIN_ID = 7251748706
...
def lummi_search(search_type, query):
    ...
    query_map = {
        "Illustrations": f"type:illustration AND {query}",
        "3D Models": f"type:3d_model AND {query}",
        "Styles": f"type:style AND {query}"
    }
    
    payload = {
        "query": query_map.get(search_type, query),
        "page": 1,
        "per_page": 10
    }
...
def handle_text(message):
    ...
    if user_id in user_states and user_states[user_id] == "waiting_for_search":
        ...
        if search_type:
            ...
            if not results:
                bot.send_message(message.chat.id, "لم يتم العثور على نتائج للبحث: " + query)
                return

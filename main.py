...
paid_mode = False
referral_codes = {}
notified_users = set()  # Track users we've notified about
...
def is_user_subscribed(user_id):
    try:
        ...
        # Notify admin about new subscribers
        if user_id not in notified_users:
            notify_admin_new_subscriber(user_id, channel)
...
def notify_admin_new_subscriber(user_id, channel):
    """Notify admin about new channel subscribers"""
    try:
        user_info = bot.get_chat(user_id)
        username = f"@{user_info.username}" if user_info.username else f"User ID: {user_id}"
        message = (
            f"📢 عضو جديد انضم إلى القنوات!\n\n"
            f"👤 المستخدم: {username}\n"
            f"🆔 ID: {user_id}\n"
            f"📌 القناة: {channel}\n"
        ...
        logger.info(f"Notified admin about new subscriber: {user_id} in {channel}")
    except Exception as e:
        logger.error(f"Error notifying admin: {e}")

def get_lumni_images(query):
    """Get images from Lumni API"""
    endpoint = "https://api.lumni.ai/v1/images/search"
    headers = {
        "Authorization": f"Bearer {LUMNI_API_KEY}",
        ...
        for item in data.get("data", []):
            if "url" in item:
                results.append({
                    ...
                    "source": "Lumni",
                    "attribution": item.get("attributionUrl", "")
                })
        
        logger.info(f"Found {len(results)} images for query: {query}")
        return results
    except Exception as e:
        logger.error(f"Lumni API error: {e}")
        return []
...
def send_welcome(message):
    ...
    if not is_user_subscribed(user_id):
        ...
        sent_msg = bot.send_message(message.chat.id, welcome_msg, reply_markup=keyboard)
        
        # Store message ID for later deletion
        users_db[user_id]["welcome_msg_id"] = sent_msg.message_id
    else:
        show_main_menu(message.chat.id, user_id)

def show_main_menu(chat_id, user_id):
    # Delete welcome message if exists
    if user_id in users_db and "welcome_msg_id" in users_db[user_id]:
        try:
            bot.delete_message(chat_id, users_db[user_id]["welcome_msg_id"])
            del users_db[user_id]["welcome_msg_id"]
        except:
            pass
...
def callback_query(call):
    ...
    try:
        if call.data == "check_subscription":
            ...
            bot.edit_message_text("أدخل كلمة البحث للحصول على الصور:", 
                                  call.message.chat.id, 
                                  call.message.message_id,
                                  reply_markup=None)
            ...
            show_result(call.message, query, index, user_id)
...
def process_search_query(message):
    ...
    loading_msg = bot.send_message(message.chat.id, f"🔍 جاري البحث عن الصور لكلمة: {query}...")
    
    # Get results from Lumni API
    try:
        results = get_lumni_images(query)
    except Exception as e:
...
def show_result(loading_msg, index, user_id):
    ...
    row = []
    if index > 0:
        row.append(types.InlineKeyboardButton(text="«« السابق", callback_data=f"result_{index-1}_{query}"))
    row.append(types.InlineKeyboardButton(text=f"{index+1}/{len(results)}", callback_data="no_action"))
    if index < len(results) - 1:
        row.append(types.InlineKeyboardButton(text="التالي »»", callback_data=f"result_{index+1}_{query}"))
    if row:
        keyboard.row(*row)
    
    # Download button
    keyboard.add(types.InlineKeyboardButton(text="◤تحميل◥", callback_data=f"download_{url}_{query}"))
    ...
    try:
        ...
        try:
            bot.edit_message_text(f"✅ تم العثور على {len(results)} صورة. انقر لرؤية المحتوى:\n\n{title}", 
                                 loading_msg.chat.id, 
                                 loading_msg.message_id,
                                 reply_markup=keyboard)
        except:
            bot.send_message(loading_msg.chat.id, f"✅ تم العثور على {len(results)} صورة. انقر لرؤية المحتوى:\n\n{title}",
                            reply_markup=keyboard)

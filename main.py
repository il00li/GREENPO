...
def run_bot():
    ...
    if setup_webhook():
        ...
        if is_port_in_use(port):
            logging.error(f"Port {port} is already in use. Trying to find an available port...")
        ...
        try:
            ...
            # المحاولة على منفذ آخر
            try:
                serve(app, host='0.0.0.0', port=0)  # 0 يعني اختيار منفذ عشوائي
            except Exception as e2:
                logging.error(f"Failed to start server on random port: {e2}")
    else:

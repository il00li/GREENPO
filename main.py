...
import os
import time
from urllib.parse import quote  # استبدال url_quote من werkzeug

app = Flask(__name__)

# تكوين البوت
TOKEN = "8110119856:AAGbhhB7lyxEH5D6i9brtcJhyHJQkLCu3tI"
ADMIN_ID = 7251748706
...
def lummi_search(search_type):
    ...
    payload = {
        "query": query_map.get(search_type, ""),
        "page": 1,
        "per_page": 10
    }
...
def callback_handler(call):
    ...
    if call.data == "check_subs":
        ...
        results = lummi_search(search_type)
        
        if not results:
            bot.answer_callback_query(call.id, "لم يتم العثور على نتائج", show_alert=True)
            return
            
        users_data[user_id]["searches"] = users_data.get(user_id, {}).get("searches", 0) + 1
        search_results[user_id] = {"type": search_type, "results": results, "index": 0}
        send_result(user_id, chat_id, message_id, 0)
...
"""
        ...
        total_searches = sum([u["searches"] for u in users_data.values()])
...
def download_media(user_id, index):
    ...
    # إرسال إلى القناة
    caption = f"تم التحميل بواسطة: @{bot.get_chat(user_id).username}\nالعنوان: {title}"

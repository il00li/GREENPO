...
def run_file_async(file_path, process_key, chat_id, user_id, file_name, is_permanent=False, message_id=None):
    try:
...
"""
        ...
        # تحديث الرسالة الأصلية بدلاً من إرسال رسالة جديدة
        if message_id:
            try:
        ...
        error_message = f"{get_random_emoji()} حدث خطأ أثناء تشغيل الملف: {str(e)}"
        if message_id:
            try:
                bot.edit_message_text(
                    error_message,
    ...
    try:
        ...
        # خيارات التجديد
            ...
            try:
                ...
                bot.edit_message_text(
                    f"{get_random_emoji()} فشل في تجديد اشترак المستخدم {target_user_id}",
                    chat_id,
                    message_id
                )
            ...
            if len(parts) >= 4:
                ...
                # تحديث الرسالة لإظهار أن التشغيل بدأ
                bot.edit_message_text(
                    f"{get_random_emoji()} جاري تشغيل الملف: {file_name}...",
                    chat_id,
                    message_id
                )
                ...
                if os.path.exists(file_path):
                    ...
                    # تشغيل الملف بشكل غير متزامن (عادي) مع تمرير معرف الرسالة للتحديث
                    process_executor.submit(
                        run_file_async, 
                        file_path, 
                        process_key,
                        ...
                        file_name,
                        False,
                        message_id  # تمرير معرف الرسالة للتحديث
                    )
                    
                    bot.answer_callback_query(call.id, f"{get_random_emoji()} جاري التشغيل العادي...")
                else:
                    bot.edit_message_text(
                        f"{get_random_emoji()} الملف غير موجود: {file_name}",
                        chat_id,
                        message_id
                    )
            ...
            if len(parts) >= 4:
                ...
                # تحديث الرسالة لإظهار أن التشغيل بدأ
                bot.edit_message_text(
                    f"{get_random_emoji()} جاري تشغيل الملف بشكل دائم: {file_name}...",
                    chat_id,
                    message_id
                )
                ...
                if os.path.exists(file_path):
                    ...
                    # تشغيل الملف بشكل غير متزامن (دائم) مع تمرير معرف الرسالة للتحديث
                    process_executor.submit(
                        run_file_async, 
                        file_path, 
                        process_key,

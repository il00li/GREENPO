...
def get_lumni_content(query, media_type):
    """Get content from Lumni API for all media types"""
    # In a real implementation, this would use the actual Lumni API
    # For demonstration, we'll simulate a reliable API response
    try:
        # Simulate API request delay
        time.sleep(1)
        
        # Generate sample results based on media type
        results = []
        media_types = {
            "video": "فيديو",
            "icon": "أيقونة",
        ...
        arabic_type = media_types.get(media_type, media_type)
        
        for i in range(1, 6):
            # For videos, use sample video URLs
            if media_type == "video":
                video_id = f"{query}-{i}"
                video_url = f"https://sample-videos.com/video{random.choice(['123', '321', '456'])}/mp4/720/{video_id}.mp4"
                results.append({
                    "id": i,
                    "url": video_url,
                    "title": f"{arabic_type}: {query} #{i}",
                    "type": media_type,
                    "source": "Lumni"
                })
            # For other media types, use image URLs
            else:
                img_url = f"https://source.unsplash.com/600x400/?{query.replace(' ', '-')}-{i}"
                results.append({
                    "id": i,
                    "url": img_url,
                    "title": f"{arabic_type}: {query} #{i}",
        ...
        logger.info(f"Generated {len(results)} results for '{query}' ({media_type})")
        return results
    except Exception as e:
        logger.error(f"Error generating Lumni content: {e}")
        return []

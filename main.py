...
# تكوين البوت
TOKEN = "8110119856:AAHd8yHv54d50GzdvSeRz3h0XvixPu4rg-I"
ADMIN_ID = 7251748706
...
def lummi_search(search_type, query):
    ...
    # بناء استعلام البحث حسب الوثائق الرسمية
    # بعد دراسة الوثائق: نستخدم استعلامات مخصصة لكل نوع
    query_map = {
    ...
    base_query = query_map.get(search_type, "")
    full_query = f"{base_query} AND {query}" if query else base_query
    ...
    try:
        print(f"Searching Lummi API for: {full_query}")
        ...
        if response.status_code == 200:
            ...
            error_msg = f"Lummi API error: {response.status_code} - {response.text}"
            print(error_msg)
            return []
    except Exception as e:
        error_msg = f"Lummi search exception: {str(e)}"
        print(error_msg)
        return []
...
def create_search_menu():
    ...
    markup.add(
        ...
        InlineKeyboardButton("🔙 رجوع", callback_data="back_main")  # زر الرجوع المطلوب
    )
    return markup
...
def send_result(user_id, chat_id, message_id, index):
    ...
    item = results[index]
    # استخدام الحقل الصحيح للوسائط حسب وثائق Lummi
    # بعد الدراسة: بعض النتائج تستخدم "preview_url" وبعضها "url"
    media_url = item.get("preview_url") or item.get("url") or item.get("media_url")

...
# مصادر البحث المفتوحة المصدر لكل نوع
SEARCH_SOURCES = {
    "Illustrations": [
        {"name": "Pixabay", "url": "https://pixabay.com/api/", "key": "38004513-5b4d1f7d9f8c4f5b3d0e7d3d0"},
        {"name": "OpenClipart", "url": "https://openclipart.org/search/json/", "key": ""},
        {"name": "Unsplash", "url": "https://api.unsplash.com/search/photos", "key": "qZgXm1V9aYq0Lz4XKJ9v4f4c3j9JkX7Q2W3bX4jK4"}
    ],
    "3D Models": [
        {"name": "Sketchfab", "url": "https://api.sketchfab.com/v3/search", "key": "b2edb8d8d0f44a8db6a8d8d0f44a8db6"},
        {"name": "Free3D", "url": "https://free3d.com/api/v1/search", "key": ""}
    ],
    "Styles": [
        {"name": "ArtStation", "url": "https://www.artstation.com/api/v2/search/projects.json", "key": ""},
        {"name": "DeviantArt", "url": "https://www.deviantart.com/api/v1/oauth2/browse/popular", "key": "29342"}
    ]
}
...
def search_assets(search_type, query):
    sources = SEARCH_SOURCES.get(search_type, [])
    if not sources:
        return []
    
    # اختيار مصدر عشوائي للتنوع
    source = random.choice(sources)
    print(f"Using source: {source['name']} for {search_type} search")
    
    results = []
    
    try:
        if source['name'] == "Pixabay":
            params = {
                "key": source['key'],
                "q": query,
                "image_type": "vector" if search_type == "Illustrations" else "all",
                "per_page": 10
            }
        ...
        elif source['name'] == "OpenClipart":
            params = {"query": query}
            response = requests.get(source['url'], params=params, timeout=15)
            if response.status_code == 200:
                data = response.json()
                for item in data.get('payload', [])[:10]:
                    results.append({
                        "title": item.get('title', 'No title'),
                        "media_url": f"https://openclipart.org{item.get('svg', {}).get('url', '')}",
                        "source": "OpenClipart"
                    })
        
        elif source['name'] == "Unsplash":
            headers = {"Authorization": f"Client-ID {source['key']}"}
            params = {"query": query, "per_page": 10}
            response = requests.get(source['url'], headers=headers, params=params, timeout=15)
            if response.status_code == 200:
                data = response.json()
                for item in data.get('results', []):
                    results.append({
                        "title": item.get('description', 'No title') or item.get('alt_description', 'No title'),
                        "media_url": item.get('urls', {}).get('regular', ''),
                        "source": "Unsplash"
                    })
            ...
            if response.status_code == 200:
                ...
                for item in data.get('data', []):
                    results.append({
                        "title": item.get('title', 'No title'),
                        "media_url": item.get('cover_url', ''),
                        "source": "ArtStation"
                    })
        
        elif source['name'] == "DeviantArt":
            params = {
                "q": query,
                "limit": 10,
            ...
            if response.status_code == 200:
                ...
                for item in data.get('results', []):
                    results.append({
                        "title": item.get('title', 'No title'),
                        "media_url": item.get('content', {}).get('src', ''),
                        "source": "DeviantArt"
                    })
        
        print(f"Found {len(results)} results from {source['name']}")
        return results
...
def handle_text(message):
    ...
    if user_id in user_states and user_states[user_id] == "waiting_for_search":
        ...
        if search_type:
            # إجراء البحث
            query = message.text
            bot.send_chat_action(chat_id, 'typing')  # إظهار حالة الكتابة
            
            results = search_assets(search_type, query)
            
            if not results:
                bot.send_message(chat_id, "⚠️ لم يتم العثور على نتائج للبحث: " + query)
            ...
            # إرسال أول نتيجة
            send_result(user_id, chat_id, None, 0)
...
def callback_handler(call):
    ...
    if call.data == "check_subs":
        ...
        try:
            ...
            try:
                bot.send_message(chat_id, "اختر نوع البحث:", reply_markup=create_search_menu())
            except:
                pass
...
def send_result(user_id, chat_id, message_id, index):
    ...
    title = item.get("title", f"نتيجة البحث #{index+1}")
    ...
    try:
        if media_url:
            ...
            # إذا لم يكن هناك media_url
            if message_id:
                try:
                    bot.delete_message(chat_id, message_id)
                except:
                    pass
            bot.send_message(chat_id, f"{caption}\n(لا يتوفر رابط مباشر)", reply_markup=markup)
    except Exception as e:
        error_msg = f"Error sending result: {str(e)}"
        print(error_msg)
        try:
            bot.send_message(chat_id, f"⚠️ خطأ في عرض النتيجة: {str(e)}")
        except:
            pass

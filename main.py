...
# إعدادات البوت
BOT_TOKEN = '8215456582:AAHFB0z4Gx3ANHgVNOOm1XgvBPMmlQHEO6A'
...
def bot_status(message):
    ...
    gemini_status = "🟢 متاح" if GEMINI_AVAILABLE else "🔴 غير متاح"
...
def handle_buttons(message):
    ...
    if any(emoji in text for emoji in FRUIT_EMOJIS):
    ...
    if text == 'رفع ملف':
        ...
        if not GEMINI_AVAILABLE:
            bot.send_message(chat_id, f"{get_random_fruit()} عذراً، خدمة الذكاء الاصطناعي غير متاحة حالياً.")
            return
            
        # بدء جلسة محادثة جديدة
        ai_sessions[user_id] = gemini_model.start_chat(history=[])
        bot.send_message(chat_id, f"{get_random_fruit()} مرحباً! أنا مساعد Gemini AI. كيف يمكنني مساعدتك اليوم؟\n\nأرسل رسالتك وسأرد عليك.")
...
@bot.message_handler(func=lambda message: message.from_user.id in ai_sessions)
def handle_ai_conversation(message):
    ...
    wait_msg = bot.send_message(chat_id, f"{get_random_fruit()} جاري معالجة طلبك...")
    
    try:
        # إرسال الرسالة إلى Gemini AI
        response = ai_sessions[user_id].send_message(user_message)
        
        # إرسال الرد
        bot.edit_message_text(
            f"{get_random_fruit()} **رد الذكاء الاصطناعي:**\n\n{response.text}",
            chat_id,
            wait_msg.message_id
        )
        ...
        bot.edit_message_text(
            f"{get_random_fruit()} عذراً، حدث خطأ أثناء معالجة طلبك: {str(e)}",
            chat_id,
            wait_msg.message_id
        )
        del ai_sessions[user_id]

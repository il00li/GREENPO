...
def get_lumni_content(query, content_type):
    """Get content from Lumni API for all types except videos and icons"""
    # This is a placeholder implementation - replace with actual Lumni API call
    # Actual API endpoint and parameters should be obtained from Lumni documentation
    
    # Simulating API response with different content types
    results = []
    
    # For demo purposes, we'll generate sample content
    for i in range(1, 6):
        if content_type == "image":
            url = f"https://dummyimage.com/600x400/3498db/ffffff&text={query}+{i}"
        elif content_type == "illustration":
            url = f"https://dummyimage.com/600x400/2ecc71/ffffff&text={query}+{i}"
        elif content_type == "3d":
            url = f"https://dummyimage.com/600x400/9b59b6/ffffff&text=3D+{query}+{i}"
        else:
            continue
...
def process_search_query(message, media_type):
    ...
    # Show first result
    show_result(loading_msg, media_type, query, 0, user_id)

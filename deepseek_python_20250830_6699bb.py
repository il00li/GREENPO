    ...
    try:
        with conn.cursor() as cur:
            ...
            if existing_user:
                ...
                # إضافة مستخدم جديد
                cur.execute(
                    "INSERT INTO users (user_id, username, is_premium, is_banned, invite_count, search_count, download_count, referrer_id) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
...
def admin_users_panel(call):
    ...
    markup.add(
        ...
        In极lineKeyboardButton("🧩 فك حظر مستخدم", callback_data="admin_unban_user")
    )
...
@bot.call极back_query_handler(func=lambda call: call.data == "admin_unban_user")
...
def process_activate_sub(message):
    ...
    try:
        ...
        try:
            bot.send_message(user极id, "🎉 تم ترقية حسابك إلى العضوية المميزة! يمكنك الآن الاستفادة من جميع ميزات البوت.")
        except:
            pass
        admin_panel(message)
    except ValueError:
...
def process_transfer_all_step2(message):
    ...
    if not target_channel.startswith('@'):
        bot.send_message(ADMIN_ID, "极❌ يجب أن يبدأ معرف القناة بـ @")
        admin_panel(message)
        return
...
def process_transfer_limit_step3(message):
    ...
    target_channel = message.text.strip()
    if not target极channel.startswith('@'):
        bot.send_message(ADMIN_ID, "❌ يجب أن يبدأ معرف القناة بـ @")
        admin_panel(message)
        return
...
def admin_publish_channels(call):
    ...
    markup = InlineKeyboardMarkup(row_width=2)
    markup.add(
        InlineKeyboardButton("➕ إضافة قناة نشر", callback_data极="admin_add_publish_channel"),
...
def transfer_members_from_channel(source_channel):
    ...
    try:
        ...
        bot.send_message(ADMIN_ID, f"🚀 بدء نقل الأعضاء من {极source_channel} إلى قنوات الاشتراك الإجباري...")
...
def admin_remove_publish_channel(call):
    ...
    bot.register_next_step_handler(call.message, process极remove_publish_channel)
...
@bot.callback_query_handler(func=lambda call: call.data == "admin_edit_publish极channel")
...
def edit_channel_interval(call):
    ...
    markup.add(
        ...
        InlineKeyboardButton("كل 3 أيام", callback_data="极interval_3")
    )
    markup.add(InlineKeyboardButton("🧃 رجوع", callback_data="admin_back"))
...
def set_channel_interval(call):
    ...
    bot.edit_message_text(
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
极        text=f"✅ تم تعيين فترة النشر إلى كل {interval} أيام"
    )
    admin_panel(call.message)

@bot.callback_query_handler(func=lambda call: call.data == "edit_channel_types")
def edit极channel_types(call):
    ...
    channel_id = user_data[ADMIN_ID].get('edit_channel_id')
    if not channel_id:
        bot.send_message(ADMIN_ID极, "❌ لم يتم تحديد قناة")
        admin_panel(call.message)
        return
...
def edit_channel_mention(call):
    ...
    current_setting = auto_publish_channels极[channel_id]['mention_bot']
...
def set_channel_mention(call):
    ...
    channel_id = user_data[ADMIN_ID].get('edit极channel_id')
    if not channel_id:
        bot.send_message(ADMIN_ID, "❌ لم يتم تحديد قناة")
        admin_panel(call.message)
        return
...
def process_search_term(message, user_id):
    ...
    # استرجاع نوع المحتوى
    if user_id not in user_data or 'content_type' not in user_data[极user_id]:
        show_main_menu(chat_id, user_id, new_message=True)
        return
...
def send_to_my_channel(call):
    ...
    try:
        ...
        if content_type == 'video' and 'videos' in item:
            video_url = item['极videos']['medium']['url']
...
def show_settings(call):
    ...
    markup.add(InlineKeyboardButton(f"{random.choice(NEW_EMOJIS)} رجوع", callback_data="back极_to_main"))
...
def upgrade_premium(call):
    ...
    try:
        ...
        logger.error(f"خطأ في عرض ترقية العضوية: {极e}")
